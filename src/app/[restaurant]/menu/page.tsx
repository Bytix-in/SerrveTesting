...
export default function PublicMenuPage() {
  ...
  // Order placement function with authentication and payment integration
  const placeOrderWithAuth = async (customerInfo: CustomerInfo, user: User | null) => {
    if (!restaurant || cart.length === 0) return

    try {
      setOrderLoading(true)

      // Create payment order with user information
      const orderPayload = {
        ...
        user_id: user?.id || null, // Include authenticated user ID
        items: cart.map(item => ({
      ...
      console.log('Placing order with authentication:', { 
        user_id: user?.id, 
      ...
      if (paymentData.payment_session_id) {
        try {
          console.log('Initializing Cashfree payment with session ID:', paymentData.payment_session_id)
          ...
          if (typeof window !== 'undefined' && (window as any).Cashfree) {
            ...
            console.log('Cashfree SDK not available, using fallback...')
            // Fallback: redirect to payment page
            window.location.href = `https://payments-test.cashfree.com/order/#${paymentData.payment_session_id}`
          }
        } catch (err) {
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      {/* New Authentication-Enabled Checkout */}
      {showCheckout && restaurant && (
        <Checkout
          cart={cart}
          restaurant={restaurant}
          onClose={() => setShowCheckout(false)}
          onPlaceOrder={placeOrderWithAuth}
